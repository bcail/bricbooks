...
def _load_data(storage, many_txns):
    ...
    mortgage = Account(type_=pft.AccountType.LIABILITY, name='Mortgage')
    storage.save_account(mortgage)
    credit_card = Account(type_=pft.AccountType.LIABILITY, name='Credit Card')
    ...
    car_insurance = Account(type_=pft.AccountType.EXPENSE, user_id='420', name='Car Insurance', parent=transportation)
    ...
    mortgage_interest = Account(type_=pft.AccountType.EXPENSE, user_id='520', name='Mortgage Interest', parent=housing)
    storage.save_account(mortgage_interest)
    medical = Account(type_=pft.AccountType.EXPENSE, user_id='600', name='Medical')
    storage.save_account(medical)
    taxes = Account(type_=pft.AccountType.EXPENSE, user_id='700', name='Taxes')
